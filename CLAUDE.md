...
**PILAR ERP** — Sistema SaaS ERP para Ecuador con cumplimiento de facturación electrónica SRI.

- **Especificación**: `foundation/` y `modules/` (documentación completa de arquitectura y módulos)
- **Flutter app**: `source-flutter/app/` (proyecto activo — estructura core + features implementadas)
- **Paquetes locales**: `source-flutter/paquetes/` (`fluent_ui` fork, `fluent_ui_reactive`, `flutter-auth-fluent-ui`)
...
- **Tema**: `FluentThemeData` con `AccentColor` del SO (`system_theme`) o personalizado por empresa
...
`PilarShell` usa `NavigationView` de fluent_ui con `PaneDisplayMode.auto`. No hay widget custom de navegación — fluent_ui lo resuelve solo.

**Breakpoints de contenido** (definidos en `pilar_breakpoints.dart`):

| Nombre | Ancho | Navegación | Contenido |
|--------|-------|-----------|-----------|
| mobile | < 600px | Drawer (hamburguesa) | ListView + cards |
| tablet | 600–900px | Rail (iconos) | ListView o 2 cols máx |
| desktop | 900–1200px | Sidebar expandido | `SfDataGrid` |
| large | > 1200px | Sidebar expandido | `SfDataGrid` + panel detalle |

**Tema**: `FluentThemeData` con `accentColor` del SO (via `system_theme`) o color de empresa. Providers: `pilarThemeProvider`, `pilarDarkThemeProvider`, `themeBrightnessProvider`, `empresaColorProvider`.
...
│   │   ├── theme_provider.dart        ← re-export de pilar_theme.dart
│   │   ├── alertas_provider.dart
│   │   └── perfil_provider.dart       ← PerfilUsuario + perfilUsuarioProvider (perfil por empresa)
│   ├── router/
│   │   └── app_router.dart            ← GoRouter con ShellRoute → PilarShell
│   ├── services/
│   │   └── window_service.dart        ← persistencia geometría ventana desktop
│   ├── shell/
│   │   ├── pilar_shell.dart           ← NavigationView + WindowListener
│   │   └── pilar_header.dart          ← acciones derechas del TitleBar
│   └── theme/
│       ├── pilar_theme.dart           ← FluentThemeData + providers de tema
│       ├── pilar_breakpoints.dart     ← PilarBreakpoints + DeviceSize enum
│       └── pilar_spacing.dart         ← constantes de espaciado base 4px
└── features/
    ├── auth/screens/                  ← login_screen, reset_password_screen
    ├── select_empresa/screens/        ← select_empresa_screen
    ├── onboarding/screens/            ← onboarding_wizard, onboarding_wizard_screen
    ├── dashboard/                     ← dashboard_screen, module_launcher_grid
    ├── administracion/                ← admin_panel, empresa, usuarios, modulos, configuracion
    ├── alertas/widgets/               ← alertas_panel
    ├── perfil/screens/                ← perfil_screen.dart (perfil del usuario por empresa)
    └── notificaciones/                ← notificaciones_provider, notificaciones_dialog
```
...
│   │   ├── config.toml
│   │   ├── migrations/       ← 000–029 foundation + mod_*.sql ensamblados
│   │   └── functions/        ← Edge Functions ensambladas (foundation + módulos)
│   │       ├── _shared/      ← cors.ts
│   │       ├── auth-setup-handler/  ← crea bucket Storage al crear empresa
│   │       ├── invite-user/         ← invitaciones A (existente) y B (nueva)
│   │       ├── upload-logo/         ← sube logo empresa a Storage
│   │       └── import-data/         ← importador de datos con templates
...
│
├── modules/
│   ├── infraestructura/      ← administracion/ + comunicacion/
│   ├── core/                 ← entidades/ + facturacion/ + ventas/ + compras/
│   │                            + inventario/ + contabilidad/ + tesoreria/ + tributacion/
│   └── extensiones/
│       ├── facturacion_ec/   ← SRI Ecuador: XML V2.1.0, XAdES-BES, RIDE, SOAP
│       ├── tributacion_ec/   ← ATS, F-103, F-104 + regulatorio/
│       ├── ia/               ← pgvector, embeddings, chat
│       ├── pagos-online/     ← Kushki/Paymentez/PayPhone
│       ├── citas-belleza/    ← App Salón 3 flavors
│       └── [pos/ ecommerce/ rrhh/ crm/ proyectos/ activos-fijos/ ...]
│
├── source-flutter/
│   ├── app/                  ← Proyecto Flutter principal (activo)
│   └── paquetes/             ← fluent_ui (fork), fluent_ui_reactive, flutter-auth-fluent-ui
│
├── scripts/
...
> **Fuente canónica**: `foundation/` y `modules/`. No existe directorio `docs/`.
...
| `foundation/arquitectura.md` | Patrones capas, Riverpod, Brick, RLS, multi-tenancy, PilarShell real |
...
| `foundation/apis/rpcs-por-modulo.md` | 85+ RPCs documentadas; 🇪🇨 = Ecuador-specific |
| `foundation/integraciones/pagos.md` | Patrón adaptador de pagos genérico |
...
| Archivo | Contenido |
|---------|-----------|
| `modules/extensiones/facturacion_ec/arquitectura-sri.md` | Pipeline SRI: librerías, Storage buckets, ambientes |
| `modules/extensiones/facturacion_ec/flujos-sri.md` | Flujos XAdES-BES, SOAP SRI, RIDE PDF, nota crédito, ATS |
| `modules/extensiones/tributacion_ec/regulatorio/` | XSD SRI, schemas XML/PDF, normativa fiscal Ecuador |
| `modules/extensiones/pagos-online/integraciones/pasarelas-ecuador.md` | Pasarelas, débito bancario, datáfonos, QR |

### Módulos Core — `module.md` por módulo

| Módulo | Spec | Tablas clave |
|--------|------|--------------|
| **Entidades** | `modules/core/entidades/module.md` | `contactos`, `productos`, `listas_precios` |
...
- **fluent_ui**: sistema de diseño principal — `FluentApp.router` + `FluentThemeData` + `NavigationView`. `FluentTheme.of(context)` en widgets, NUNCA `Theme.of(context)`, NUNCA `MaterialApp`
- **Tema**: `PilarTheme.build()` genera `FluentThemeData` con `AccentColor` del SO (`system_theme`) o color de empresa. NUNCA hardcodear colores
- **Navegación**: `NavigationPane` con `PaneDisplayMode.auto` — sin widget custom. `PaneItemExpander` para grupos, `PaneItem` para ítems. El índice seleccionado mapea la ruta go_router actual
- **Layout responsive**: `LayoutBuilder` + `PilarBreakpoints` para elegir entre `SfDataGrid` (≥900px) y `ListView`/cards (<900px). NUNCA grids en pantallas <600px
