 ...
 * Auth:   Authorization: Bearer <JWT del usuario>
 *
 * IMPORTANTE: deployada con verify_jwt=false porque custom_access_token_hook
 ...
 * intacta: el JWT se valida manualmente → authClient con RLS aplica normalmente.
 *
 * Responde:
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  // ── 1. Validar Authorization header (seguridad manual, verify_jwt=false) ──
  ...
  // ── 3. Leer cuenta usando el JWT del usuario (respeta RLS) ────────────────
  ...
  // ── 4. Construir la URL del webhook ───────────────────────────────────────
  const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
  const webhookUrl =
    `${supabaseUrl}/functions/v1/com-telegram-webhook?account_id=${account_id}`;

  // ── 5. Llamar setWebhook en la Bot API de Telegram ────────────────────────
  ...
  // ── 6. Actualizar meta_json con el resultado (admin client, bypass RLS) ───
