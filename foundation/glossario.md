...
### PilarNavigation
Sistema de navegación estilo Odoo implementado con los componentes de `fluent_ui`. Jerarquía: `PilarModule > PilarMenuGroup > PilarMenuItem` (3 niveles). Se implementa con `NavigationPane` usando `PaneItem`, `PaneItemExpander` (grupos colapsables) y `PaneItemHeader` (separadores de sección). El footer del pane contiene acceso a Configuración y Perfil. `PaneDisplayMode.auto` resuelve el layout por breakpoint sin código manual: expanded en desktop, compact en tablet, minimal (hamburguesa) en móvil.
