...
PRINCIPIO ARQUITECTÓNICO:
  El empleado ES un contacto. No existe tabla "empleados".
  empleado = contactos WHERE es_empleado = true
           + campos laborales añadidos por rrhh/001_extend_contactos.sql
           + tablas operativas (contratos, nómina, asistencia) con empleado_id → contactos

  Una misma persona puede ser simultáneamente empleado, cliente y proveedor
  sin duplicar su cédula/RUC en múltiples tablas.
```

```sql
-- ══════════════════════════════════════════════════════════════════
-- EXTENSIÓN DE contactos (por rrhh/001_extend_contactos.sql)
-- ══════════════════════════════════════════════════════════════════
-- NO hay tabla "empleados". Los campos laborales van en contactos:

contactos  [extendida por RRHH]
  -- Campos base (foundation + entidades):
  es_empleado             BOOLEAN DEFAULT false   -- activa el rol de empleado
  -- Campos añadidos por RRHH (001_extend_contactos.sql):
  codigo_empleado         VARCHAR(20)             -- P-001, EMP-042
  user_id                 UUID FK -> auth.users   -- acceso al sistema (opcional)
  departamento_id         UUID soft -> departamentos
  posicion_id             UUID soft -> cargos     -- posición formal en organigrama
  jefe_directo_id         UUID soft -> contactos  -- otro empleado
  establecimiento_id_rrhh UUID soft -> establecimientos
  fecha_ingreso           DATE
  fecha_salida            DATE                    -- NULL = activo
  motivo_salida           VARCHAR(50)
  sueldo_actual           DECIMAL(14,2)
  bono_fijo               DECIMAL(14,2) DEFAULT 0
  numero_afiliacion_iess  VARCHAR(20)
  tipo_afiliacion         VARCHAR(20) DEFAULT 'DEPENDENCIA'
  fondos_reserva_acumular BOOLEAN DEFAULT false
  proyeccion_gastos_ir    DECIMAL(14,2) DEFAULT 0
  cargas_familiares       INTEGER DEFAULT 0
  banco_nomina            VARCHAR(100)
  tipo_cuenta_nomina      VARCHAR(20)
  numero_cuenta_nomina    VARCHAR(30)
...
departamentos
  ...
  departamento_padre_id UUID soft -> departamentos  -- jerarquia/organigrama
  responsable_id  UUID soft -> contactos            -- jefe (es_empleado=true)
  centro_costo_codigo VARCHAR(20)                   -- soft ref a cuentas_contables
...
-- ══════════════════════════════════════════════════════════════════
-- [ELIMINADO: tabla empleados]
-- Los datos del empleado están en contactos (ver arriba).
-- Todas las tablas operativas usan: empleado_id UUID FK -> contactos
-- ══════════════════════════════════════════════════════════════════
  ...
  jefe_directo_id UUID FK -> contactos  -- empleado (es_empleado=true)        -- Para aprobaciones y organigrama
  -- Datos laborales
...
contratos
  id              UUID PK
  empresa_id      UUID FK -> empresas
  empleado_id     UUID FK -> contactos  -- empleado (es_empleado=true)
...
intercambios_turno
  id                UUID PK
  empresa_id        UUID FK -> empresas
  solicitante_id    UUID FK -> contactos  -- empleado (es_empleado=true)        -- Empleado que pide el intercambio
  receptor_id       UUID FK -> contactos  -- empleado (es_empleado=true)        -- Empleado que acepta cubrir
...
log_biometrico
  ...
  empleado_id     UUID FK -> contactos  -- empleado (es_empleado=true)    -- NULL si no se pudo identificar
...
CREATE TABLE evaluaciones_desempeno (
  ...
  empleado_id UUID FK -> contactos  -- empleado (es_empleado=true), evaluador_id UUID FK -> contactos  -- empleado (es_empleado=true),
...
CREATE TABLE solicitudes_empleado (
  id UUID PK, empresa_id UUID FK, empleado_id UUID FK -> contactos  -- empleado (es_empleado=true),
...
CREATE TABLE asistencia_capacitacion (
  ...
  empleado_id UUID FK -> contactos  -- empleado (es_empleado=true), fecha DATE,
